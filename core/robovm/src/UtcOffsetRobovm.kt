    ...
    public actual companion object {
        ...
        internal fun ofSeconds(seconds: Int): UtcOffset {
            ...
            return if (seconds % (15 * SECONDS_PER_MINUTE) == 0) {
                utcOffsetCache.get()[seconds] ?: UtcOffset(totalSeconds = seconds).also { utcOffsetCache.get()[seconds] = it }
            } else {
...
private var utcOffsetCache: ThreadLocal<MutableMap<Int, UtcOffset>> = object: ThreadLocal<MutableMap<Int, UtcOffset>>() {
    override fun initialValue(): MutableMap<Int, UtcOffset> {
        return mutableMapOf()
    }
}
